...
class SpatioTemporalPosEmbeddingHelper(VerboseNNModule):
    def __init__(
        ...
        if self.learnable:
            ...
            # self.pos_embed = self.pos_embed.to(dtype=torch.float16)
            # if is_deepspeed_zero3_enabled:
            #     import deepspeed
            #     with deepspeed.zero.GatheredParameters(self.pos_embed):
            #         trunc_normal_(self.pos_embed, std=0.02)
            # else:
            # trunc_normal_(self.pos_embed, std=0.02)
            # self.pos_embed = self.pos_embed.to(dtype=dtype_ori)
        else:
...
class RGBDTPreprocessor(VerboseNNModule):
    def __init__(
        ...
        if is_deepspeed_zero3_enabled:
            ...
            with deepspeed.zero.GatheredParameters(stem.parameters()):
...
def build_causal_attention_mask(context_length):
    ...
    mask = torch.full((context_length, context_length), float("-inf"), device="cuda")
