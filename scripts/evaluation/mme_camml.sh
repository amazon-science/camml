#!/bin/bash

model=${1}
iclnum=${2:-3}
memory=${3:-"665k"}
CHUNKS=8
CKPT=${4}

for IDX in $(seq 0 $((CHUNKS-1))); do
  CUDA_LAUNCH_BLOCKING=1 CUDA_VISIBLE_DEVICES=$IDX python -m camml.eval.model_vqa_loader_camml \
    --model-path checkpoints/${model} \
    --question-file ./data/eval/MME/llava_mme.jsonl \
    --image-folder ./data/eval/MME/MME_Benchmark_release_version \
    --temperature 0 \
    --answers-file ./data/eval/MME/answers/${CKPT}_${CHUNKS}_${IDX}.jsonl \
    --num-chunks $CHUNKS \
    --icl_num ${iclnum} \
    --memory ${memory} \
    --chunk-idx $IDX \
    --conv-mode vicuna_v1 &
done

wait

output_file=./data/eval/MME/answers/${CKPT}.jsonl

# Clear out the output file if it exists.
> "$output_file"

# Loop through the indices and concatenate each file.
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat ./data/eval/MME/answers/${CKPT}_${CHUNKS}_${IDX}.jsonl >> "$output_file"
done


cd ./data/eval/MME

python convert_answer_to_mme.py --experiment ${CKPT}

cd eval_tool

python calculation.py --results_dir answers/${CKPT}